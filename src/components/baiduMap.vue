...
function ready({BMap: BMapInstance, map: mapInstance}: any) {
  ...
  mapType.value = new BMap.MapType('newmap', defaultTileLayer, {minZoom: 15, maxZoom: 18});
  ...
  if (pointArr.value.length > 1) {
    ...
    for (let j = 0; j < all_sensor.value.length; j++) {
      for (let k = 0; k < all_sensor.value[j].length; k++) {
        addMark(SENSOR, SIZE, [all_sensor.value[j][k].lng, all_sensor.value[j][k].lat])
...
<!--    <bm-map-type :map-types="['BMAP_NORMAL_MAP', 'BMAP_HYBRID_MAP']" anchor="BMAP_ANCHOR_TOP_LEFT"></bm-map-type>-->
  ...
  <el-button @click="addMark(SENSOR, SIZE, [113.80848202641658, 22.816506505348503])">传感器</el-button>
  <el-button @click="addMark(GATEWAY, SIZE, [113.80868202641658, 22.816506505348503])">网关</el-button>
  <el-button @click="addMark(CROSSING, SIZE, [113.80848202641658, 22.816906505348503])">路口</el-button>
